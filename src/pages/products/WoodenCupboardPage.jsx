...
const features = [
  { icon: '🪵', title: 'Solid Teak-wood', desc: 'Built from genuine Sagwan wood with natural strength and termite resistance.' },
  { icon: '📐', title: 'Custom Dimensions', desc: 'Designed to fit your exact room size — floor-to-ceiling or wall-to-wall.' },
  { icon: '👕', title: 'Smart Organisation', desc: 'Shelves, drawers, hanging rods, shoe racks, and accessory trays inside.' },
  { icon: '🪞', title: 'Mirror & Finish', desc: 'Full-length mirror options with premium polish and laminate finishes.' },
  { icon: '🛡️', title: '5 Year Warranty', desc: 'Covers structural defects, hardware, and sliding mechanism issues.' },
  { icon: '🔧', title: 'Expert Installation', desc: 'Professional fitting by our team with clean, damage-free setup.' },
];

const stats = [
  { title: '700+', sub: 'Wardrobes Made' },
  { title: 'SINCE', sub: '2014' },
  { title: '100%', sub: 'Custom Made' },
  { title: '5 YR', sub: 'Warranty' },
];

const WoodenCupboardPage = () => {
  ...
  return (
    <div ref={pageRef}>
      <PageBanner desktopImage="https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?w=1920&q=80" mobileImage="https://images.unsplash.com/photo-1618221195710-dd6b41faaea6?w=800&q=80" overlayText="CUPBOARD" />
            ...
            <div className="w-full xl:w-1/2 flex items-center content-center flex-wrap py-10 sm:py-[45px] xl:py-[25px] px-5 min-[468px]:px-8 sm:px-10 md:px-[40px] lg:px-[50px] xl:px-[30px] justify-center" style={fadeStyle('intro', 0.15)}>
              <div className="w-full">
                <h2 className="font-['Mulish',sans-serif] font-normal uppercase text-black text-[22px] leading-[30px] min-[468px]:text-[28px] min-[468px]:leading-normal sm:text-[30px] xl:text-[28px] min-[1700px]:text-[32px] m-0 p-0 mb-4">
                  WOODEN <span className="text-[#814882] font-semibold">CUPBOARD</span>
                </h2>
                <p className="m-0 text-[#4c4c4c] text-[15px] leading-[25px] sm:text-base sm:leading-[26px] mb-8">
                  Our wooden cupboards and wardrobes are crafted from solid Teak-wood with fully customised interiors. Swing door, sliding, walk-in, or loft — we build every style to fit your room dimensions and storage needs. Premium hardware, soft-close hinges, and elegant finishes come standard.
                </p>
      ...
      <section className="py-[25px] sm:py-[45px] bg-white overflow-hidden" data-anim="types">
        <div className="section-head mb-[25px] sm:mb-[35px]"><h2>CUPBOARD STYLES</h2></div>
        <div className="w-full px-5 md:px-[60px] min-[1441px]:px-[100px] mx-auto">
          <div className="flex flex-wrap gap-[3px] sm:gap-[6px]">
            {cupboardTypes.map((type, idx) => (
      ...
      <section className="py-[35px] sm:py-[50px] lg:py-[60px] bg-[#f8f8f8] overflow-hidden" data-anim="cta">
        <div className="w-full px-5 md:px-[60px] min-[1441px]:px-[100px] mx-auto text-center" style={fadeStyle('cta')}>
          <h2 className="font-normal uppercase text-black text-[22px] min-[468px]:text-[28px] sm:text-[30px] lg:text-[34px] min-[1390px]:text-[38px] mb-4" style={{ fontFamily: "'Mulish', sans-serif" }}>
            ORGANISE YOUR SPACE WITH <span className="text-[#814882] font-semibold">CUSTOM WARDROBES</span>
          </h2>
          <p className="text-[#4c4c4c] text-[14px] sm:text-[15px] leading-[1.8] max-w-[600px] mx-auto mb-8">Share your room dimensions — we'll design a wardrobe that fits perfectly and stores everything neatly.</p>
...
export default WoodenCupboardPage;
