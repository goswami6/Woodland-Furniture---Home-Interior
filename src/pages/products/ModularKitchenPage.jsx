...
const galleryImages = [
  { src: 'https://images.unsplash.com/photo-1622372738946-62e02505feb3?w=800&q=80', alt: 'L-Shape modular kitchen' },
  { src: 'https://images.unsplash.com/photo-1600489000022-c2086d79f9d4?w=800&q=80', alt: 'U-Shape modular kitchen' },
  { src: 'https://images.unsplash.com/photo-1570222094114-d054a817e56b?w=800&q=80', alt: 'Parallel modular kitchen' },
  { src: 'https://images.unsplash.com/photo-1556185781-a47769abb7ee?w=800&q=80', alt: 'Island kitchen design' },
  { src: 'https://images.unsplash.com/photo-1604709177225-055f99402ea3?w=800&q=80', alt: 'Straight modular kitchen' },
  { src: 'https://images.unsplash.com/photo-1588854337236-6889d631faa8?w=800&q=80', alt: 'Modern kitchen interior' },
];

const kitchenTypes = [
  { name: 'L-SHAPE', img: 'https://images.unsplash.com/photo-1622372738946-62e02505feb3?w=600&q=80' },
  { name: 'U-SHAPE', img: 'https://images.unsplash.com/photo-1600489000022-c2086d79f9d4?w=600&q=80' },
  { name: 'STRAIGHT', img: 'https://images.unsplash.com/photo-1570222094114-d054a817e56b?w=600&q=80' },
  { name: 'PARALLEL', img: 'https://images.unsplash.com/photo-1556185781-a47769abb7ee?w=600&q=80' },
  { name: 'ISLAND', img: 'https://images.unsplash.com/photo-1604709177225-055f99402ea3?w=600&q=80' },
];

const features = [
  { icon: '📐', title: 'Custom Layouts', desc: 'L-shape, U-shape, Straight, Parallel & Island layouts designed to fit your space perfectly.' },
  { icon: '🪵', title: 'Premium Materials', desc: 'Built with Teak-wood and Commercial Plywood for lasting durability.' },
  { icon: '🎨', title: '3D Design Preview', desc: 'See your kitchen in 3D before we start, so you know exactly what to expect.' },
  { icon: '⚙️', title: 'Smart Storage', desc: 'Corner units, pull-out drawers, tall units and carousel systems to maximise space.' },
  { icon: '🛡️', title: 'Quality Assurance', desc: 'Every kitchen is crafted with rigorous quality checks at each stage.' },
  { icon: '🔧', title: 'Expert Installation', desc: 'Professional installation by our experienced team with minimal disruption.' },
];

const stats = [
  { title: '500+', sub: 'Kitchens' },
  { title: 'SINCE', sub: '2014' },
  { title: '100%', sub: 'Custom Made' },
  { title: '3D', sub: 'Design Preview' },
];
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (id === 'stats') {
              setTimeout(() => setActiveStatIdx(0), stats.length * 120 + 600);
            }
  ...
  return (
    <div ref={pageRef}>
        ...
        <div className="w-full max-w-full px-5 min-[468px]:px-8 sm:px-10 md:px-[80px] lg:px-[100px] mx-auto xl:px-0">
          <div className="flex flex-wrap">
            <div className="w-full xl:w-1/2 max-[468px]:hidden" style={fadeStyle('intro', 0)}>
              <img
                src="https://images.unsplash.com/photo-1622372738946-62e02505feb3?w=900&q=80"
                alt="Modular Kitchen"
            ...
            <div
              className="w-full xl:w-1/2 flex items-center content-center flex-wrap py-10 sm:py-[45px] xl:py-[25px] px-5 min-[468px]:px-8 sm:px-10 md:px-[40px] lg:px-[50px] xl:px-[30px] justify-center"
              style={fadeStyle('intro', 0.15)}
            >
              <div className="w-full">
                <h2
                  className="font-['Mulish',sans-serif] font-normal uppercase text-black text-[22px] leading-[30px] min-[468px]:text-[28px] min-[468px]:leading-normal sm:text-[30px] xl:text-[28px] min-[1700px]:text-[32px] m-0 p-0 mb-4"
                >
                  MODULAR <span className="text-[#814882] font-semibold">KITCHEN</span>
                </h2>
                <p className="m-0 text-[#4c4c4c] text-[15px] leading-[25px] sm:text-base sm:leading-[26px] mb-8">
                  At Woodland Furniture, we design and build fully customised modular kitchens that combine aesthetics with functionality. From space planning to 3D visualisation and expert installation, we handle every detail. Our kitchens are crafted with Teak-wood (Sagwan) and Commercial Plywood, ensuring durability and a premium finish that lasts for years.
                </p>
                <button onClick={() => setShowModal(true)} className="btn-comn hoveranim">
        ...
        <div className="w-full px-5 md:px-[60px] min-[1441px]:px-[100px] mx-auto">
          <ul className="flex sm:flex-wrap sm:justify-center overflow-x-auto sm:overflow-x-visible gap-[10px] sm:gap-0 snap-x snap-mandatory scrollbar-hide pb-2 sm:pb-0">
            {stats.map((stat, index) => (
              <li
                key={index}
                className="p-[5px] flex-shrink-0 w-[110px] min-[390px]:w-[125px] sm:flex-shrink sm:w-1/4 lg:px-[14px] snap-center"
                style={fadeStyle('stats', index * 0.12)}
              >
                <div
                  className={`group rounded-full w-[104px] h-[104px] min-[360px]:w-[110px] min-[360px]:h-[110px] min-[390px]:w-[118px] min-[390px]:h-[118px] sm:w-[134px] sm:h-[134px] xl:w-[150px] xl:h-[150px] mx-auto flex items-center justify-center cursor-pointer transition-all duration-700 ease-in-out ${activeStatIdx === index
                    ? 'bg-[#814882] shadow-none'
                    : 'bg-white shadow-[inset_0_0_0_2px_#d5d5d5,inset_0_2px_4px_rgba(0,0,0,0.08)] hover:bg-[#814882]'
                    }`}
                >
                  <div className="text-center">
                    <div className={`font-semibold mb-[3px] text-[14px] min-[390px]:text-[15px] sm:text-[17px] xl:text-[18px] uppercase transition-colors duration-700 group-hover:text-white ${activeStatIdx === index ? 'text-white' : 'text-[#814882]'
                      }`}>{stat.title}</div>
                    <div className={`text-[11px] min-[390px]:text-[12px] sm:text-[13px] xl:text-[14px] transition-colors duration-700 group-hover:text-white leading-tight ${activeStatIdx === index ? 'text-white' : 'text-[#333]'
                      }`}>{stat.sub}</div>
                  </div>
                </div>
              </li>
        ...
        <div className="w-full px-5 md:px-[60px] min-[1441px]:px-[100px] mx-auto">
          <div className="flex flex-wrap gap-[3px] sm:gap-[6px]">
            {kitchenTypes.map((type, idx) => (
              <div
                key={idx}
                className={idx === 0 ? 'w-full' : 'w-[calc(50%-3px)] sm:w-[calc(50%-6px)]'}
                style={fadeStyle('types', idx * 0.15)}
              >
                <Link to="/contact" className="block h-[155px] sm:h-[280px] md:h-[320px] min-[1681px]:h-[380px] relative overflow-hidden group bg-white">
                  <img
                    ...
                    loading="lazy"
                  />
                  <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-black text-white uppercase transition-colors duration-500 group-hover:text-[#814882] z-[1] text-[24px] min-[390px]:text-[28px] min-[468px]:text-[36px] sm:text-[42px] md:text-[52px] lg:text-[64px]" style={{ fontFamily: "'Mulish', sans-serif" }}>
                    {type.name}
                  </div>
                </Link>
              </div>
      ...
      <section className="py-[25px] sm:py-[45px] lg:py-[60px] bg-[#f8f8f8] overflow-hidden" data-anim="feat">
        <div className="section-head section-head-gray mb-[25px] sm:mb-[35px]">
          <h2>WHY CHOOSE US</h2>
        </div>
        <div className="w-full px-5 md:px-[60px] min-[1441px]:px-[100px] mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-[15px] sm:gap-[20px]">
            {features.map((f, i) => (
              <div
                key={i}
                className="bg-white p-[20px] sm:p-[25px] shadow-[0_3px_25px_rgba(0,0,0,0.06)] hover:shadow-[0_3px_25px_rgba(0,0,0,0.12)] transition-shadow duration-300 rounded-[5px]"
                style={fadeStyle('feat', i * 0.1)}
              >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-[6px] sm:gap-[10px]">
            {galleryImages.map((img, i) => (
              <div
                ...
                style={fadeStyle('gallery', i * 0.08)}
                onClick={() => setLightbox(i)}
              >
                <img
                  src={img.src}
                  alt={img.alt}
                  className="w-full h-[140px] sm:h-[220px] md:h-[280px] lg:h-[320px] object-cover transition-all duration-500 group-hover:scale-110 group-hover:opacity-80"
                  loading="lazy"
                />
              </div>
            ))}
      ...
      <section className="py-[35px] sm:py-[50px] lg:py-[60px] bg-[#f8f8f8] overflow-hidden" data-anim="cta">
        <div className="w-full px-5 md:px-[60px] min-[1441px]:px-[100px] mx-auto text-center" style={fadeStyle('cta')}>
          <h2
            className="font-normal uppercase text-black text-[22px] min-[468px]:text-[28px] sm:text-[30px] lg:text-[34px] min-[1390px]:text-[38px] mb-4"
            style={{ fontFamily: "'Mulish', sans-serif" }}
          >
            READY TO BUILD YOUR <span className="text-[#814882] font-semibold">DREAM KITCHEN?</span>
          </h2>
          <p className="text-[#4c4c4c] text-[14px] sm:text-[15px] leading-[1.8] max-w-[600px] mx-auto mb-8">
            Book a free consultation with our design team. We'll visit your site, understand your needs, and present a 3D design.
          </p>
          <button onClick={() => setShowModal(true)} className="btn-comn hoveranim">
        ...
        <div className="fixed inset-0 z-[1055] flex items-center justify-center bg-black/70" onClick={(e) => { if (e.target === e.currentTarget) setShowModal(false); }}>
          <div className="bg-white/82 rounded-[5px] w-[90%] max-w-[500px] relative overflow-hidden">
            ...
            <div className="bg-[#814882] px-5 py-3.5 flex justify-between items-center">
              ...
              <button onClick={() => setShowModal(false)} className="bg-transparent border-none cursor-pointer flex items-center justify-center">
                <svg width="14" height="14" viewBox="0 0 16 16" fill="#fff"><path d="M.293.293a1 1 0 011.414 0L8 6.586 14.293.293a1 1 0 111.414 1.414L9.414 8l6.293 6.293a1 1 0 01-1.414 1.414L8 9.414l-6.293 6.293a1 1 0 01-1.414-1.414L6.586 8 .293 1.707a1 1 0 010-1.414z" /></svg>
              </button>
            </div>
            <div className="pt-5 px-6 pb-4 text-center">
              <p className="text-[#4c4c4c] text-sm mb-4">Fill out the enquiry below and we will get back to you</p>
            </div>
